...
// Function to handle modifying item name and quantity
function modifyItem(tdElement) {
    // Create a form element
    const form = document.createElement('form');
    
    // Create an input field for the new item name
    const input = document.createElement('input');
    input.type = 'text'; // Change type to "text"
    input.value = tdElement.textContent; // Populate input with current item name
    ...
    tdElement.innerHTML = ''; // Clear existing content
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Get new item name from input field
        const newItemName = input.value;

        // Update table data content with new item name
        tdElement.textContent = newItemName;
...
function handleTableDataClick(event) {
    ...
    if (event.target.tagName === 'TD') {
        // Call modifyItem function to allow item name modification
